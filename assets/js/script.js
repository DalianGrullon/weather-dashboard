...
function renderWeather(city, currentWeather, forecastWeather) {
  ...
  for (let i = 0; i < forecastWeather.length; i++) {
    ...
    futureForecastEl.children[i].children[0].children[2].children[0].textContent = `Temp: ${forecastWeather[i].temp}°F`;
...
function retrieve() {
  if (localStorage.length > 0) {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      // let prevSearchObj = JSON.parse(localStorage.getItem(localStorage.key(i)));
      // renderWeather();
    }
  } else {
    return
